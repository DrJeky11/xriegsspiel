...
**Agent-training follow-up, 2026-09-16:** the owner requested a local training and Sensei roadmap on `agent-training`, selected **teaching the exercise and explaining decisions**, then authorized training a **teaching-hint policy**, evaluated by the owner and an instructor. The [plan](../design/agent-training-plan.md) records the broader roadmap. The [first training run](../training/guided-policy-v1.md) records an actual CPU-trained hint selector, replay/parity checks, an optional practice pilot and 32 review examples. Teaching effectiveness and stronger learned opposition remain unverified.
