...
**Observed from baseline `f25a7a7` on 2026-09-16, before workspace consolidation:** links to retired source below pin that baseline.

| Existing component | Useful foundation | Gap for analysis or training |
| --- | --- | --- |
| [Island session at baseline](https://github.com/DrJeky11/xriegsspiel/blob/f25a7a7a02cd4fec40f427db1732da01ab94e829/server/session.ts) | Accepted command ID, expected revision, action and resulting state; flushed JSONL append before acknowledgment; retry protection | Startup restores stored snapshots and checks version strings, without replay-validating every journal transition. No participant/run identity or decision timestamps. |
...
| [Island UI at baseline](https://github.com/DrJeky11/xriegsspiel/blob/f25a7a7a02cd4fec40f427db1732da01ab94e829/src/main.ts), [geographic UI at baseline](https://github.com/DrJeky11/xriegsspiel/blob/f25a7a7a02cd4fec40f427db1732da01ab94e829/src/scenario/main.ts) | Local previews/cancellation, explicit confirmation, browser exports, runtime diagnostics | Previews, hints, cancellations and XR lifecycle are not persisted as an analysis record. An export timestamp does not date the original decisions. |
...
Start edits in [the geographic session handler](../../server/scenario-session.ts), [map-session registry](../../server/map-sessions.ts) and [HTTP service](../../server/main.ts), preserving pure rules in [the rules module](../../src/scenario/rules.ts). Add a small persistence module only as needed to share transaction/replay behavior. Browser and Quest capture should attach to common command/preview callbacks in [the shared workspace](../../src/play/workspace.ts). Cover all six independent map sessions without restoring the retired Island application.
...
- At the original baseline, ran `npm test`: **37 tests passed**, including existing durability, duplicate/stale handling and geographic replay/import checks. These validate that baseline foundation, not the proposed collector.
