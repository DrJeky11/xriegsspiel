...
For each action retain run/scenario/rules versions, round, role, observation ID, command ID, subject ID, CP and pressure before/after, acceptance, causal event IDs, objective transfers, incident attribution and resulting metrics. The offline calculator checks terminal ledgers only. The [playable runtime](../ai-opponent.md) additionally replays accepted joint orders and validates checkpoints; its record uses full per-round observations and command IDs. Arbitrary referee edits and a separate observation-ID dataset schema remain future extensions.

**Runtime clarifications, 2026-09-16:** changing a movement's declared lead cargo does not change the challenge target (vessel plus destination). Handling resolves before movement, with start-round eligibility. Blue has first priority on odd rounds and Red on even rounds for duplicate-rescue custody and simultaneous competing coordination acceptances. SPR-H01's optional `short-window/1` variant makes its two transports available in round 3; the baseline remains ready in round 1. Variants are explicit and independent of policy difficulty.
