...
async function main() {
  ...
  let selected=catalog.pieces.find(p=>p.name.startsWith('M1A2 Abrams'))!.id,page=0;
  ...
  function renderList() {
    ...
    get('list').innerHTML=list.slice(page*30,(page+1)*30).map(p=>`<button class="entry ${p.id===selected?'selected':''}" aria-pressed="${p.id===selected}" data-entry="${p.id}"><strong>${esc(p.name)}</strong><span>${esc(equipment.get(p.equipmentId)!.domain)} · ${p.kind} · ${p.eraEvidence.reportedYear??'date unknown'}</span><small>${eligible(p)?'Catalog eligible · service unverified':'Review needed for this year / force'}</small></button>`).join('')||'<p class="empty">No matching equipment. Try a broader search or another force.</p>';
  ...
  function renderDetails() {
    ...
    const selectedForce=value('force')==='all'?(p.forceEvidence.find(m=>lab.eligibility(p.id,m.force,year()).allowed)?.force??p.forceEvidence[0].force):force();
