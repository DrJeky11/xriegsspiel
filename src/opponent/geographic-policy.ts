import { goalDistance, separation, cellsFor, goal, distances, geographicRoutes } from './geography.ts';
import { exitGoal } from './geographic-rules.ts';
...
import { legalCandidates, observe, planningWorld, resolveRound } from './geographic-rules.ts';

export const POLICY_VERSION = 'geographic-planner/1.0.0';
export const DIFFICULTIES: Record<Difficulty, { label: string; description: string; plans: number; replies: number; transitions: number; depth: number }> = {
  ...
  advanced: { label: 'Advanced', description: 'Looks up to three rounds ahead against several plausible opposing responses.', plans: 12, replies: 3, transitions: 108, depth: 3 },
};
...
function distance(o: Observation, start: string, end: string) {
  if (start === 'Exited') return 0;
  return Math.ceil(goalDistance(o.geography!, start, end) / o.geography!.movement);
}
function exitSector(o: Observation) { return exitGoal(o.scenario.id); }
const routeMemo = new WeakMap<Observation, Map<string, string[]>>();
function targetGoal(o: Observation, a: Asset): string | null {
  if (a.kind === 'transport' || a.cargo.includes('M1')) return a.cargo.length ? 'Outpost' : null;
  if (a.kind === 'merchant') return exitSector(o);
  if (o.scenario.id === 'SEN-H01' && a.side === 'blue') return o.metrics.handover ? null : 'Review';
  if (a.kind === 'rescue' && a.cargo.length < a.capacity) return o.items.find(i => i.kind === 'survivor' && i.completed === null)?.location ?? null;
  return null;
}
function predictedRoute(o: Observation, a: Asset) {
  let memo=routeMemo.get(o);if(!memo){memo=new Map();routeMemo.set(o,memo);}const existing=memo.get(a.id);if(existing)return existing;
  const target=targetGoal(o,a),path=a.tileId?[a.tileId]:[];
  if(target&&a.tileId){const field=distances(o.geography!.mapId,goal(o.geography!,target).tileIds),cells=cellsFor(o.geography!.mapId);
    for(let i=0;i<o.geography!.movement*2;i++){const at=path.at(-1)!,next=cells.get(at)!.neighbors.filter(n=>(field.get(n)??999)<(field.get(at)??999)).sort((a,b)=>(field.get(a)??999)-(field.get(b)??999)||a.localeCompare(b))[0];if(!next)break;path.push(next);}
  }
  memo.set(a.id,path);return path;
}
function patrolPotential(o: Observation, tileId: string) {
  const targets=o.assets.filter(a=>a.side==='blue'&&a.tileId&&(a.kind==='transport'&&a.cargo.length||a.kind==='merchant'));
  if(o.scenario.id==='SEN-H01')return -goalDistance(o.geography!,tileId,'Review')*8;
  let value=-100;
  for(const a of targets){const route=predictedRoute(o,a),nearest=distances(o.geography!.mapId,route).get(tileId)??999;
    value=Math.max(value,35-Math.min(nearest,12)*9+((distances(o.geography!.mapId,route.slice(1,o.geography!.movement+1)).get(tileId)??999)<=1?18:0));
  }
  return value;
}
function missionProgress(game: Game, side: Side, o: Observation): number {
  ...
  if (id.startsWith('SPR')) {
    ...
    for (const a of game.assets.filter(a => a.kind === 'transport')) score += (blue ? 1 : -1) * a.cargo.length * (2 - distance(o, a.tileId!, 'Outpost')) * 32;
    if (id === 'SPR-F01') {
      score += m.medical_on_time * 240;
      const medical = game.assets.find(a => a.id === 'B3')!;
      if (!m.medical_on_time && blue) score += (2 - distance(o, medical.tileId!, 'Outpost')) * (game.round >= 4 ? 130 : 75);
    }
  } else if (id === 'SEN-H01') {
    ...
    score += coordinate;
    for (const a of game.assets.filter(a => a.kind === 'rescue' && a.side === side && a.cargo.length < a.capacity)) score += (2 - distance(o, a.tileId!, 'Casualty')) * 30;
    ...
    for (const a of game.assets.filter(a => a.kind === 'merchant' && a.sector !== 'Exited' && a.sector !== 'Holding custody')) score += (blue ? 1 : -1) * (o.sectors.length - distance(o, a.tileId!, exitSector(o))) * 28;
    ...
    if (id.startsWith('BAB')) {
      ...
      if (blue) for (const group of game.items.filter(i => i.kind === 'survivor' && i.completed === null)) {
        ...
        const nearest = Math.min(4, ...available.map(a => distance(o, a.tileId!, group.location)));
  ...
  if (rescueRequirement) {
    ...
    if (completed < rescueRequirement) {
      ...
      const slots = game.assets.filter(a => a.kind === 'rescue').flatMap(a => Array.from({ length: a.capacity - a.cargo.length }, (_, i) => game.round + distance(o, a.tileId!, remaining[0].location) + i + 1)).sort((a, b) => a - b);
  ...
  if (id === 'SPR-F01' && !m.medical_on_time) {
    ...
    if (!a || game.round + distance(o, a.tileId!, 'Outpost') + 1 > 5) score -= 3000;
  }
  if (id === 'HOR-F01' && !m.welfare_complete && game.round >= 4) score -= 3000;
  if (side === 'red') score += game.assets.filter(a=>a.side==='red'&&a.tileId).reduce((n,a)=>n+patrolPotential({...o,assets:game.assets},a.tileId!)*.45,0);
...
function actionUtility(o: Observation, candidate: Candidate, style = 0): number {
  ...
  switch (order.type) {
    case 'move': {
      ...
      if(a.side==='red' && (a.kind==='patrol'||o.scenario.id==='SEN-H01'))return patrolPotential(o,order.target)-patrolPotential(o,a.tileId!)-1;
      const target=targetGoal(o,a);if(!target)return -30;
      const priority=a.cargo.includes('M1')?100:a.kind==='rescue'?70:a.kind==='transport'?50:45;
      const delta=(goalDistance(o.geography!,a.tileId,target)-goalDistance(o.geography!,order.target,target))/o.geography!.movement;
      return delta*priority+(a.id==='T1'?6:0)-(candidate.warning?(style===1?20:6):0);
    }
    ...
    case 'assure': return style === 1 ? 36 * order.effort : 16 * order.effort;
    case 'challenge': {
      const a=o.assets.find(a=>a.id===order.asset)!,target=o.assets.find(a=>a.id===order.target);
      if(order.target==='handover')return goalDistance(o.geography!,a.tileId,'Review')<=1?60:0;
      if(!target)return -30;
      const crossing=(distances(o.geography!.mapId,predictedRoute(o,target)).get(a.tileId!)??999)<=1;
      return crossing?38+order.effort*10+(target.id==='T1'?8:0):-18;
    }
    case 'hold': return 0;
  }
}
function planUtility(o: Observation, orders: Order[], style = 0) {
  ...
  for (const order of orders) if (order.type === 'assure' && !orders.some(other => 'asset' in other && other.asset === order.target && ['move','deliver','transfer','handover'].includes(other.type))) value -= 200;
...
function plans(o: Observation, limit: number, style = 0): Order[][] {
  ...
  for (let depth = 0; depth < 3; depth++) {
    ...
    for (const p of beam) for (let i = p.next; i < choices.length; i++) {
      const orders = [...p.orders, choices[i].order]; if (batchError(orders) || new Set(orders.filter(o=>o.type==='move').map(o=>o.target)).size !== orders.filter(o=>o.type==='move').length) continue;
...
export function baseline(o: Observation, kind: 'hold' | 'random' | 'greedy' | 'cautious' | 'deadline', seed = 1): Order[] {
  ...
  for (const { c } of candidates) if (!batchError([...selected, c.order]) && !(c.order.type==='move'&&selected.some(o=>o.type==='move'&&o.target===(c.order as Extract<Order,{type:'move'}>).target))) selected.push(c.order);
  return selected;
}
...
export function decide(o: Observation, difficulty: Difficulty, seed: number): PolicyDecision {
  ...
  if (difficulty === 'novice') {
    ...
    const orders: Order[] = [];
    for (const { c, value } of candidates) if (value > 0 && !batchError([...orders, c.order]) && !(c.order.type==='move'&&orders.some(o=>o.type==='move'&&o.target===(c.order as Extract<Order,{type:'move'}>).target))) {
    ...
    for (const reply of replies) {
      ...
      try {
        ...
        for (let depth = 1; depth < limits.depth && !next.finished && transitions < limits.transitions; depth++) {
          ...
          next = structuredClone(next); next.round++; next.events = []; for(const a of next.assets)a.movement=a.kind==='patrol'?next.geography!.patrolMovement:next.geography!.movement;
