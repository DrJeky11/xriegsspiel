...
export interface TablePiece { id:string; tileId:string; force:'red'|'blue'; symbol:string; name:string; model:MiniatureKind; selected:boolean; cargo:number; layer:string }
...
export class PieceLayer {
  ...
  set(pieces:TablePiece[],reachable:string[],path:string[],select:(id:string)=>void) {
    ...
    this.clearLabel();this.pieces.clear();this.models.clear();this.disposeGroup(this.routes);this.pieceHits=[];
    ...
    this.hover(pieces.find(p=>p.selected)?.id??null);
  }
  ...
  hover(id:string|null){
    ...
    lines.slice(0,4).forEach((text,i)=>c.fillText(text,384,44+i*42,730));c.font='28px sans-serif';c.fillStyle='#b9d7d7';c.fillText(`${p.id}  ·  ${p.force==='blue'?'United States':'China'}${p.cargo?' · '+p.cargo+' cargo':''}`,384,canvas.height-20,730);
